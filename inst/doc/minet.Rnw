...
information estimator explained in this section as input. All the estimators except the
gaussian one require discrete data values. The \Rfunction{disc} function 
allows the user to choose between two binning
algorithms.
 
\subsection{Supervized Discretization}

All the mutual information estimators described in this section, except the gaussian estimator, require discrete data values.
...
ew.data <- disc(syn.data,disc.method,nbins)
syn.data[1:5,1:5]
ew.data[1:5,1:5]
@
...
<<>>=
disc.method <- "equalfreq"
ef.data <- disc(syn.data,disc.method,nbins)
ef.data[1:5,1:5]
@

\subsection{Mutual Information Estimators}
The package implements five estimators, called \Robject{"empirical"},\Robject{"millermadow"},
\Robject{"dirichlet.sg"},\Robject{"shrink"} and \Robject{"gaussian"}. 
...
\subsubsection{Gaussian Estimation}
   Under gaussian assumption, the mutual information can be directly
computed by the following formula:
$$I(X;Y) = -\frac{1}{2} \log(1 - \rho_{x,y}^2) $$
with $\rho$ being the Pearson's correlation between $X_i$ and $X_j$ \cite{papier}.
   Since only the relative value of entropy is required for network inference,
using directly the $\rho^2$-matrix can be more effective. Furthermore, this avoids the
problematic case $\rho = 1$. Note that the complexity of evaluating $\rho^2$ is also in
$O(m)$ where $m$ is the number of samples.
...
Three network inference methods are available in the package : \Rfunction{aracne.net}, 
\Rfunction{clr.net} and \Rfunction{mr.net}. These receive as input the mutual information 
...
In the following code, the \Rfunction{mr.net} algorithm is applied to the mutual
...
The function \Rfunction{aracne.net} has an extra argument \Robject{eps}
...
#library(Rgraphviz)
data(syn.data)
net <- minet( dataset=syn.data, method="aracne", estimator="gaussian" )
